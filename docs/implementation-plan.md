...
Recommended v0 defaults are indentation-sensitive blocks, spaces-only
indentation, `--` line comments, ASCII canonical output, exactly two mixed
inputs, and these units:

```text
mL  L  mmol  mol  g  mol/L  M  degC  atm
```

The language owner and chemistry owner may change these defaults at Gate 0,
but parser work begins only after the resulting fixture set is reviewed.
...
| `F-003` | L+C | `F-002` | Freeze `.chems` v0 EBNF, diagnostic schema, and valid/invalid language fixtures. Both owners approve the canonical source. | 4 h |
...
- [ ] Confirm the `.chems` v0 decisions in `F-003`.
