...
`chem-kernel` owns deterministic structural elaboration and the authoritative
...
flowchart LR
    ...
    catalogue["chem-catalogue<br/>validated reference data"] --> elaborate
...
`expand_provisional` and `expand_reference` retain where facts came from. The
former accepts working data; the latter accepts packaged reviewed reference
data. Neither grants chemistry authority. Both feed the same immutable HIR and
the same kernel validation.

Both paths validate evidence packets. Runtime research is retained as
`external` provenance; it cannot self-assert chemistry authority. The HIR keeps
...
`validate_provisional` and `validate_reference` execute the identical kernel.
Both produce validated structural derivations, differing only in serialized
provenance (`provisional` or `reviewed_reference`). The derivation records
source, expansion, and catalogue identities and rejects stale reuse.

## Structural frames

Slice 6 projects any current kernel-validated structural derivation into immutable
...
Observation records separately retain `external` evidence provenance. Frame
generation has one public capability for all validated derivations; catalogue
review status neither adds nor removes renderer functionality.
...
- `CorruptReferenceData` for impossible post-catalogue structural failures.
