...
fn sequence_view(
    ...
    let stage_index = reaction_sequence::stage_index(sequence.progress);
    let controls = sequence_controls(sequence.playback);
    let stages = sequence_stages(stage_index);

    let diagram = container(
        canvas(ReactionSequenceDiagram::new(run.frames(), frame_index))
            .width(Fill)
            .height(Fill),
    )
    .style(theme::inset)
    ...
    let explanation = container(
        row![
            column![
                text(format!("State {} of {}", frame_index + 1, frames.len()))
                    .size(type_scale::BODY_LARGE)
                    .color(color::TEXT),
                text(reaction_sequence::frame_summary(frame))
...
fn sequence_stages(active_index: usize) -> Element<'static, Message> {
    reaction_sequence::STAGES
        .iter()
        .enumerate()
        .fold(
                ...
                row.push(
                    container(
                        column![
                            ...
                            text(*title).size(type_scale::CAPTION).color(
...
fn sequence_heading(
    ...
    row![
        ...
        space().width(Fill),
        column![
            text("TRUSTED KERNEL FRAME SEQUENCE")
                .size(type_scale::MICRO)
                .color(color::SUCCESS),
            text(format!("frame digest {}…", &frame_digest[..12]))
                .size(type_scale::CAPTION)
                .color(color::MUTED),
        ]
        .spacing(spacing::XXS),
...
fn composition_summary(
    ...
    let (title, detail) = if supported {
        if queued {
            (
                "Trusted reaction ready",
                "Language expansion, kernel validation, and frame projection complete".to_owned(),
            )
        } else {
            (
                "Ready to start",
    ...
    .on_press_maybe((supported && !queued).then_some(Message::StartReaction))
    .padding([spacing::XS, spacing::MD])
    .style(theme::primary_button)
    .into();
    ...
    .style(move |_| reaction_status_style(supported, queued, trigger_reveal))
    .padding([spacing::XS, spacing::SM])
    .width(Fill)
    .into()
}
...
fn reaction_participants(atoms: &[PlacedAtom]) -> Vec<DraftParticipant> {
    ...
    let compositions = groups
        .iter()
        .map(|group| DraftParticipant::Composition(group.preview.formula));
    let loose_atoms = atoms
        .iter()
        .filter(|atom| !grouped_ids.contains(&atom.id))
        .map(|atom| DraftParticipant::Atom(atom.atomic_number));
...
mod tests {
    ...
    #[test]
    fn lithium_and_water_enable_only_the_trusted_reaction_trigger() {
        let mut state = lithium_water_state();
