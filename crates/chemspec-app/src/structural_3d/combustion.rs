...
/// Initial fuel level above the bench, preserving the authored basin.
const BASIN_LEVEL: f32 = 1.13;
/// Share of the pool consumed across the reaction.
const BURN_DOWN: f32 = 0.42;
...
/// Soot film settling on the inner glass above the fuel line: a fixed
/// seeded speckle population whose opacity grows with `deposit`.
fn add_soot_deposit(
    mesh: &mut Mesh,
    surface_y: f32,
    rim_y: f32,
    ...
    for speckle in 0..SPECKLES {
        ...
        let inset = 0.94 - 0.012 * seeded_unit(seed, speckle, 403);
...
pub(super) fn add_combustion_assembly(
    ...
    // The burning pool: fuel is consumed, leaving its old waterline.
    let floor_y = layout.bench_top + 0.09;
    let initial_level = layout.bench_top + BASIN_LEVEL;
    let burn = smooth01((progress - 0.06) / 0.82);
    let level = initial_level - (initial_level - floor_y - 0.04) * BURN_DOWN * burn;
    ...
    let palette = if incomplete {
        FlamePalette::Natural
    } else {
        FlamePalette::BurnerBlue
    };
    add_surface_flame(meshes, palette, flame_source, strength, 1.6, phase, seed);
    add_ignition_sparks(
        &mut meshes.emissive,
    ...
    if incomplete {
        ...
        // Clean combustion: only a faint product plume leaves the vessel.
        add_rising_plume(
            &mut meshes.translucent,
            state.surface_centre,
            strength * 0.8,
