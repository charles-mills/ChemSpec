...
fn draw_structure_labels(
    ...
    let (label, atoms) = match operation {
        StructuralOperation::AssociateIonic { left, right } => {
            ("Ionic association", vec![left.as_str(), right.as_str()])
        }
        StructuralOperation::AssignProduct { atoms, .. } => (
            "Validated product",
        ...
        StructuralOperation::CleaveCovalent { left, right, .. } => {
            ("Covalent bond cleaved", vec![left.as_str(), right.as_str()])
        }
        StructuralOperation::FormCovalent { left, right, .. } => {
            ("Shared electron pair", vec![left.as_str(), right.as_str()])
        }
    };
    if let Some(target) = average_position(atoms, positions) {
        let anchor = Point::new(bounds.width * 0.74, bounds.height * 0.16);
        draw_connector_label(frame, label, anchor, target, IONIC);
    }
}
...
fn layout(frame: &StructuralFrame, bounds: Rectangle) -> BTreeMap<String, Point> {
    ...
    if let Some(StructuralOperation::AssociateIonic { left, right }) = &frame.active_operation
...
fn draw_atom(frame: &mut canvas::Frame, atom: &AtomState, center: Point, active: bool) {
